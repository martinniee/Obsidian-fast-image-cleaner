...
export default class MyPlugin extends Plugin {
		...
		this.register(
			this.onElement(
				...
				()=>{
					...
					// console.log("鼠标移出")
					// if(this.isMouseoutButton()){
						// const btn_delete = document.querySelector(".btn-delete") as HTMLButtonElement;
	 ...
	 * @returns 
	 */
	isMouseoutButton = (): boolean=>{
		const btn_delete = document.querySelector(".btn-delete") as HTMLButtonElement;
		btn_delete.onmouseover = function(){
			console.log("鼠标移入到按钮上了");
			return false;
		}
		return true;
	}
	 ...
	 * @returns 删除按钮元素的对象
	 */
	getDynamicDeleteButton = (): HTMLButtonElement => {
		const btn_delete: HTMLButtonElement = document.createElement("button");
	...
	onMouseover(event: MouseEvent) {
		...
		switch (imgType) {
			...
			case "img": {
				...
				// 🚩--------获取动态的图片删除按钮--------
				const btn_delete = this.getDynamicDeleteButton();
				console.log("btn_delete:  " + btn_delete);
				// 将动态生成的按钮添加到img元素的父级div元素下作为第一个子元素
				imageDOM.parentElement?.insertBefore(btn_delete as HTMLButtonElement,imageDOM);
